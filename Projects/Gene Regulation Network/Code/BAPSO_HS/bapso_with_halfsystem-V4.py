...
def HS(data,gene_index,position,num_gene,index):
   delta_t = 6
   fij= position[:4]
   delta_i = position[-3]
 ...
 #  gene_regulator_expression = [gene_tp_1[i] for i in combo]
   ...
   for t in range(1,len(data)):
     ...
     gene_predicted.append(delta_t*prod_term+(1-(delta_t*epsilon_i))*gene_tp_1[gene_index])
...
def BAPSO(num_gene,num_particle,max_iteration,accel_factor):
    ...
    for gene in range(num_gene):
      ...
      for index in range(num_particle):
        ...
        fitness,matrix = HS(data,gene,position,num_gene,index)
      ...
      for itr in range(max_iteration-1):
        ...
        for index,particle in enumerate(particles):
          ...
          fitness,matrix = HS(data,gene,particle.position,num_gene,index)
