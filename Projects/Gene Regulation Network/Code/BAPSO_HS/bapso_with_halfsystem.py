...
def HS(data,gene_index,position,num_gene):
   ...
   for combo in combinations:
      ...
      for t in range(1,len(data)):
        ...
        d_i = data.iloc[t-1].values[gene_index+1] - gene_predicted[-1]
...
def plot_fitness(fitness_per_gene, max_iteration,cur):
    ...
    plt_filename = 'fitness_evolution_BAPSO_'+str(cur)+".png"
...
#  num_particle = math.comb(num_gene,num_regulator)
  ...
  for cur in range(0,1):
    ...
    random.seed(7+cur)
    result_matrix, fitness_per_gene,resultant_matrix = BAPSO(num_gene, num_particle, max_iteration, max_inertia, min_inertia, accel_factor)
