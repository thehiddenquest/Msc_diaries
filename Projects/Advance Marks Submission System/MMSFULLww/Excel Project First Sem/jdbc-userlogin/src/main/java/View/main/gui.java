...
public class gui extends javax.swing.JFrame {
	...
	private void register() {
		boolean flag = loginAndRegister.registerInfo();
		if (flag)
			showMessage(Message.MessageType.SUCCESS, "User name added successfully");
		else
			showMessage(Message.MessageType.ERROR, "User name not added");
	}

	private void login() {
		user  = loginAndRegister.loginInfo();
		if (user.getFlag()) {
			showMessage(Message.MessageType.SUCCESS, "User Authenticated");
			try {
				Thread.sleep(2000);
			...
			setVisible(false);
			animator.stop();
			new mainGui(user.getUserName(),user.getEmailId()).setVisible(true);
		}
		else
			showMessage(Message.MessageType.ERROR, "Authentication Failed");
	}
	...
	private void initComponents() {
		...
		bgLayout.setHorizontalGroup(bgLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0,
				933, Short.MAX_VALUE));
		bgLayout.setVerticalGroup(bgLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 537,
				Short.MAX_VALUE));
