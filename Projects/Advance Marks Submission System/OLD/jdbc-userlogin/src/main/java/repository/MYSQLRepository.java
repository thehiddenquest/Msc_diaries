...
public class MYSQLRepository<T> extends storageRepository<T> {
	...
	private T retrieveMarks(String[] operation) {
		...
		if (operation[0].equals("31")) {
			String query = "SELECT coll, cate, number1, total_marks\n" + "FROM (\n"
					+ "    SELECT e.coll, e.cate, e.number1, SUM(m.marks_obt) AS total_marks\n" + "    FROM examDB e \n"
					+ "    INNER JOIN marksDB m ON e.coll = m.coll AND e.cate = m.cate \n"
					+ "                         AND e.number1 = m.number1 AND e.paper_type = m.paper_type \n"
					+ "    WHERE e.stream = ?\n" + "    AND e.sem = ? \n" + "    AND e.year = ? \n"
					+ "    AND e.subject = ?\n" + "    GROUP BY e.coll, e.cate, e.number1\n"
					+ ") AS total_marks_table\n" + "WHERE total_marks = (\n" + "    SELECT MAX(total_marks)\n"
			...
			try {
				...
				while (result.next()) {
					...
					marksTO.setCate(result.getString("cate"));
					marksTO.setNumber(result.getString("number1"));
					marksTO.setMarksObt(result.getString("total_marks"));
					System.out.println("TAKE IT" + marksTO.getColl() + marksTO.getCate() + marksTO.getNumber()
							+ marksTO.getMarksObt());
					rmarks.add(marksTO);
				}
				return (T) rmarks;
